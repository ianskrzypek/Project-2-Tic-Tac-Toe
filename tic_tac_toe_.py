...
def gameend():
    ...
    global player2score
    global aiscore
    recentgames.append([board,win])
    printboard(board)
    if win == ("X"):
        print(player1name,"(X) Won The Game!")
        player1score = player1score+1
        streak["player1"] = streak["player1"] + 1
        streak["player2"] = 0
        streak["ai"] = 0
        recentgames[-1].append(player1name)
    elif win ==("O"):
        if twoplayer == 1:
...
def results():
    ...
    try:
        if len(recentgames) > 0:
            ...
            print("you must first play a game to review its results")
    except:
